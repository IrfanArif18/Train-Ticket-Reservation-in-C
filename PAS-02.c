/*
TRAIN TICKET RESERVATION C PROGRAM

This progarm is coded by Group 2 of Dasar Komputer-03 Class
in order to fulfill End of 3rd Semester Project

Group 2 members:
-Irfan Arif Maulana  (1906379270)
-Muhammad Nur Ismail (1906379346)
...
#define QTTY 30 // Number of trains in the database

#define PASS_STR(arg)   #arg
// Modify these two macros' arguments for user's USERNAME and PASSWORD
#define GET_UNAME PASS_STR(hello)
#define GET_PWORD PASS_STR(world123)
...
train trn[QTTY];    // Trains' database declaration
...
void getData(user* pUsr, train* pTrn) { 
    ...
    /* Train 3 */
    strcpy((pTrn + i)->trName, "Ranggajati");
...
void login(user* pUsr) {
    ...
    while(1) {
        ...
        printf("WELCOME TO SINGULIN.COM\n\n\n");
...
void printTrain(train* pTrn) {
    ...
    switch(pTrn->orig) {
        case 1: strcpy(temp, "Jakarta");
                break;
        case 2: strcpy(temp, "Bandung");
                break;
        case 3: strcpy(temp, "Semarang");
                break;
        case 4: strcpy(temp, "Yogyakarta");
                break;
        case 5: strcpy(temp, "Surabaya");
                break;
...
void calcCost(user* pUsr) {
    ...
    if(pUsr->class == 1) {
        pUsr->cost = 300000 * pUsr->numPsg;
    } else {
        pUsr->cost = 100000 * pUsr->numPsg;
    }
    
    /* Adds extra price based on train's company */
    if(strcmp(pUsr->usrTrn.trName, "Ranggajati") == 0) {
        pUsr->cost += 100000;
    } else if(strcmp(pUsr->usrTrn.trName, "Gumarang") == 0) {
        pUsr->cost += 80000;
    } else if(strcmp(pUsr->usrTrn.trName, "Taksaka") == 0) {
        pUsr->cost += 75000;
    } else if(strcmp(pUsr->usrTrn.trName, "Turangga") == 0) {
        pUsr->cost += 65000;
    } else if(strcmp(pUsr->usrTrn.trName, "Lodaya") == 0) {
        pUsr->cost += 55000;
    } else if(strcmp(pUsr->usrTrn.trName, "Malabar") == 0) {
        pUsr->cost += 50000;
    } else if(strcmp(pUsr->usrTrn.trName, "Ciremai") == 0) {
        pUsr->cost += 45000;
    } else if(strcmp(pUsr->usrTrn.trName, "Sancaka") == 0) {
        pUsr->cost += 40000;
    } else if(strcmp(pUsr->usrTrn.trName, "Bima") == 0) {
        pUsr->cost += 40000;
    }
